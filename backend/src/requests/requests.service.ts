...
// This file is deprecated. Use VehicleRequestService from './vehicle/vehicle-request.service' instead.
  ...
  async findHistoryForUser(userId: string): Promise<any[]> {
    ...
    for (const request of requests) {
      ...
      const requesterName = typeof requester === 'object' && requester !== null && 'name' in requester 
        ...
        : undefined;
      const related = this.isUserRelatedToRequest(request as any, normalizedUserId);
      ...
      for (const action of request.actionHistory) {
        ...
        const includeEntry = performerId === normalizedUserId || related;
        if (!includeEntry) {
          continue;
        }
