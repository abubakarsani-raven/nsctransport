...
class CreateIctRequestScreen extends StatefulWidget {
  const CreateIctRequestScreen({super.key, this.requestId, this.onMenuPressed});

  final String? requestId;
  final VoidCallback? onMenuPressed;

  @override
  State<CreateIctRequestScreen> createState() => _CreateIctRequestScreenState();
}

class _CreateIctRequestScreenState extends State<CreateIctRequestScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadRequestData() async {
    ...
    try {
      final request = await _apiService.getIctRequest(widget.requestId!);
      setState(() {
  ...
  Future<void> _submitRequest() async {
    ...
    try {
      final success = isEditMode
          ? await Provider.of<IctRequestsProvider>(context, listen: false)
        ...
        if (isEditMode) {
          ...
          if (request['status'] == 'needs_correction') {
            ...
            if (shouldResubmit == true && mounted) {
              try {
                final resubmitSuccess = await Provider.of<IctRequestsProvider>(context, listen: false)
          ...
          _showSuccessSnackBar('ICT request created successfully');
          await Future.delayed(const Duration(milliseconds: 500));
          await Provider.of<IctRequestsProvider>(context, listen: false).loadRequests();
          if (mounted) {
            Navigator.pop(context);
  ...
  Widget build(BuildContext context) {
    ...
    return AppScaffold(
      header: AppBar(
        title: Text(widget.requestId == null ? 'Create ICT Request' : 'Edit ICT Request'),
                  ...
                  children: [
                    TextFormField(
                      ...
                      decoration: InputDecoration(
                        labelText: 'Item *',
                        hintText: 'e.g., Laptop, Monitor, Software License',
